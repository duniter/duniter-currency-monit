...
module.exports = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // get lg file
        const LANG = getLang(`${__dirname}/../lg/gaussianWotQuality_${req.query.lg || constants2_1.MonitConstants.DEFAULT_LANGUAGE}.txt`);
        ...
        let membersList = yield dataFinder.getMembers();
        // Si les données de qualité n'ont jamais été calculés, le faire
        if (lastUpgradeTimeDatas == 0 || (lastUpgradeTimeDatas + constants2_1.MonitConstants.MIN_WOT_QUALITY_CACHE_UPDATE_FREQ) < (Math.floor(Date.now() / 1000)) || (previousNextYn != nextYn)) {
            // Calculer dSen
            ...
            membersQuality(constants2_1.MonitConstants.QUALITY_CACHE_ACTION.INIT, 0, dSen, conf.stepMax, conf.xpercent, wot.memCopy());
        }
        // Mettre a jour previousNextYn
        previousNextYn = (nextYn == "yes") ? "yes" : "no";
        // Calculer nbSentries, limit1 and label
        const nbSentries = (sentries == "no") ? membersList.length : membersQuality(constants2_1.MonitConstants.QUALITY_CACHE_ACTION.GET_SENTRIES_COUNT);
        let limit1 = 1;
        let label = LANG['QUALITY'];
        switch (unit) {
            case 'percentReached':
                limit1 = conf.xpercent * 100;
                label = LANG['PERCENT_REACHED'];
                break;
            case 'nbReached':
                limit1 = parseInt(String(conf.xpercent * nbSentries));
                label = LANG['NB_REACHED'];
                break;
            default: break;
        }
        ...
        tabMembersQuality = [];
        for (let i = 0; i < membersList.length && membersQuality(constants2_1.MonitConstants.QUALITY_CACHE_ACTION.GET_QUALITY, i) >= 0; i++) {
            if (sentries == "no") {
                tabMembersQuality[i] = membersQuality(constants2_1.MonitConstants.QUALITY_CACHE_ACTION.GET_QUALITY, i, -1);
            }
            else {
                tabMembersQuality[i] = membersQuality(constants2_1.MonitConstants.QUALITY_CACHE_ACTION.GET_QUALITY, i);
            }
        }
        ...
        if (unit == 'percentReached' || unit == 'nbReached') {
            ...
            if (unit == 'nbReached') {
                unitCoeff *= parseFloat(nbSentries);
            }
            else {
                unitCoeff *= 100.0;
            }
...
//# sourceMappingURL=gaussianWotQuality2.js.map