...
module.exports = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // get lg file
        const LANG = getLang(`${__dirname}/../lg/membersCount_${req.query.lg || constants2_1.MonitConstants.DEFAULT_LANGUAGE}.txt`);
        // get blockchain
        var blockchain = yield dataFinder.getBlockWhereMedianTimeLteNoLimit(cache.endBlock[0].medianTime);
        ...
        switch (cache.stepUnit) {
            case "hours":
                nextStepTime -= (cache.beginBlock[0].medianTime % 3600);
                break;
            case "days":
            case "weeks":
            case "months":
            case "years":
                nextStepTime -= (cache.beginBlock[0].medianTime % 86400);
                break;
            default: break;
        }
        // Calculate initial cacheIndex value
        let cacheIndex = 0;
        while (cache.blockchain[cacheIndex].number <= cache.beginBlock[0].number && (cache.blockchain.length - 1) > cacheIndex) {
            cacheIndex++;
        }
        ...
        for (let b = cache.beginBlock[0].number; b < blockchain.length; b++) {
            ...
            if (b == cache.beginBlock[0].number || (cache.stepUnit == "blocks" && bStep == cache.step) || (cache.stepUnit != "blocks" && blockchain[b].medianTime >= nextStepTime /*(tabMembersCount[tabMembersCount.length-1].timestamp+cache.stepTime)*/)) {
                ...
                while ((cache.blockchain.length - 1) > cacheIndex && cache.blockchain[cacheIndex].number <= b) {
                    cacheIndex++;
                }
                ...
                if (cache.stepUnit != "blocks") {
                    if (cache.step > 1) {
                        switch (cache.stepUnit) {
                            case "hours":
                                dateTime = previousDateTime + " - " + timestampToDatetime(blockchain[blockIndex].medianTime, cache.onlyDate);
                                break;
                            case "days":
                                dateTime = previousDateTime + " - " + timestampToDatetime(blockchain[blockIndex].medianTime - (cache.stepTime / cache.step), cache.onlyDate);
                                break;
                            case "weeks":
                                dateTime = previousDateTime + " - " + timestampToDatetime(blockchain[blockIndex].medianTime, cache.onlyDate);
                                break;
...
//# sourceMappingURL=membersCount2.js.map