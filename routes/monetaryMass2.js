...
module.exports = async (req, res, next) => {
    ...
    try {
        ...
        if (end >= begin && begin >= 1) {
            var blockchain = await dataFinder.getBlockWhereMedianTimeLteAndGteNoLimit(endBlock[0].medianTime, beginBlock[0].medianTime);
        }
        else {
            var blockchain = await dataFinder.getBlockWhereMedianTimeGtNoLimit(beginBlock[0].medianTime);
        }
