...
module.exports = (host, port, duniterServer, sigValidity, msValidity, sigWindow, idtyWindow) => {
  ...
  ***************************************/
  ...
  app.get('/about', (req, res) => co(function *() {
    try {
      ...
      htmlContent += '<div align="right">Author : <a href="https://github.com/librelois">Éloïs Librelois</a></div><br>';
  ...
  ***************************************/
  ...
  app.get('/members', (req, res) => co(function *() {
	...
	for (let m=0;m<membersList.length;m++)
	{
	  ...
	  // Calculer le nombre maximal de certifications reçues par le membre courant
	...
	{	  
	  ...
	  contenu += '>Inclure les certifications en piscine.</form><br>';
		  ...
		  // On parcourt tous les membres pour afficher ceux dont la date d'expiration est dans moins de 'd' jours
		  var nbPrintMembers = 0;
  ...
  ***************************************/
  ...
  app.get('/wasMembers', (req, res) => co(function *() {
	...
	// On parcourt tout les membres pour afficher ceux qui ont expiré
	nbExpirMembers = 0;
  ...
  ***************************************/
  ...
  app.get('/blockMembers', (req, res) => co(function *() {
	...
	for (let m=0;m<tabNbBlockByMember.length;m++)
      ...
      let blockChartMod = blockChart(req, res, HTML_MENU, tabNbBlockByMemberSort, begin, end);
  ...
  *****************************************/
  ...
  app.get('/monetaryMass', (req, res) => co(function *() {
	...
	if (blockchain[b].membersCount != previousMemberCount || blockchain[b].monetaryMass != previousMonetaryMass)
	{
      ...
      // create and fill tabRelativeMonetaryMass
      var tabRelativeMonetaryMass = []
      for (let i=0;i<tabMonetaryMass.length;i++)
      {
	tabRelativeMonetaryMass.push(tabMonetaryMass[i] / currentDividend);
	tabCurrency[i].relativeMonetaryMass = tabMonetaryMass[i] / currentDividend;
      }
      
      // Appeler le module monetaryMassChart
      let monetaryMassChartMod = monetaryMassChart(req, res, HTML_MENU, tabCurrency, begin, end);
  ...
  *****************************************/
  ...
  app.get('/membersCount', (req, res) => co(function *() {
	...
	var blockchain = yield duniterServer.dal.peerDAL.query('SELECT `issuer`,`membersCount`,`monetaryMass`,`medianTime`,`dividend`,`number`,`nonce` FROM block WHERE `fork`=0 AND `medianTime` >= '+beginBlock[0].medianTime+' ORDER BY `medianTime` ASC');
      ...
      for (let b=0;b<blockchain.length;b++)
      {
	if (blockchain[b].membersCount != previousMemberCount)
	{
      ...
      let membersCountChartMod = membersCountChart(req, res, HTML_MENU, tabMembersCount, begin, end);
