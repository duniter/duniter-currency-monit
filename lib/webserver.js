...
const monetaryMassChart = require('./monetaryMassChart.js');
const membersCountChart = require('./membersCountChart.js');
...
module.exports = (host, port, duniterServer, sigValidity, msValidity, sigWindow, idtyWindow) => {
  ...
  // Define html headers
  const HTML_HEADERS = '<!DOCTYPE html><html><head><title>ğ1-monit</title><link rel="icon" type="image/png" href="https://forum.duniter.org/uploads/default/original/1X/d0cf3f5487cd7a7fed57dd6d9c24053acf6dd8fe.ico" />'
  
  // Define html menu
  const HTML_MENU = '<table align="center" width="100%"><tr>'
    +'<td><a href="willMembers">willMembers</a></td>'
    +'<td><a href="members">members</a></td>'
    +'<td><a href="blockMembers">blockMembers</a></td>'
    +'<td><a href="monetaryMass">monetaryMass</a></td>'
    +'<td><a href="membersCount">membersCount</a></td>'
  ...
  ***************************************/
  ...
  app.get('/', (req, res) => co(function *() {
    try {
      // Define HTML content
      var htmlContent = HTML_HEADERS+'</head><body>'+HTML_MENU+'</body></html>';
  ...
  ***************************************/
  ...
  app.get('/about', (req, res) => co(function *() {
    try {
      // Define HTML content
      var htmlContent = HTML_HEADERS+'</head><body>';
      htmlContent += HTML_MENU;
      htmlContent += '<div align="center"><br><a href="https://github.com/librelois/duniter-currency-monit/blob/master/LICENSE">License AGPL V 3.0</a><br>';
      htmlContent += '<a href="https://github.com/librelois/duniter-currency-monit/">github repository</a></div><br>';
      htmlContent += '<div align="left">Author : <a href="https://github.com/librelois">Éloïs Librelois</a></div><br>';
      htmlContent += '<div align="left">Contributors : <a href="https://github.com/jytou">jytou</a> (translator)</div><br>';
  ...
  ***************************************/
  ...
  app.get('/willMembers', (req, res) => co(function *() {
    ...
    try {
      ...
      var contenu = HTML_HEADERS+'</head><body>';
      contenu = HTML_MENU;
      var identitiesList = [];
      ...
      var order = 'desc'; // Valeur par défaut
      if (typeof(req.query.d) != 'undefined' && req.query.order == 'asc') {
	order = 'asc';
      }
      var sort_by="sigCount"; // Valeur par défaut
      if (typeof(req.query.sort_by) != 'undefined') {
	sort_by = req.query.sort_by;
      }
      var hideIdtyWithZeroCert="yes"; // Valeur par défaut
      if (typeof(req.query.hideIdtyWithZeroCert) != 'undefined' && req.query.hideIdtyWithZeroCert == "no") {
	hideIdtyWithZeroCert = "no";
      }
      var sortSig = "Availability"; // Valeur par défaut
      if (typeof(req.query.sortSig) != 'undefined' && req.query.sortSig == "Creation") {
	sortSig = "Creation";
      }
	...
	lg = req.query.lg;
      ...
      // Récupérer pour chaque identité, l'ensemble des certifications qu'elle a reçu.
      var nbMaxCertifs = 0;
	  ...
	  // Calculer le nombre de certifications en attente destinées au membre courant
	  let nbPendingCertifs = tmpQueryPendingCertifsList.length;
	  let nbValidPendingCertifs = 0;
	    
	  // Récupérer les uid des émetteurs des certifications reçues par l'utilisateur
	  ...
	  for (var j=0;j<nbPendingCertifs;j++)
	  {
		...
		// Vérifier que l'identité courante n'a pas déjà reçu d'autre(s) certification(s) de la part du même membre ET dans le même état de validité du blockstamp
		...
		{
		  // Stocker la liste des certifications en piscine qui n'ont pas encore expiré
	...
	{ 
	...
	else { contenu += "<p>ERROR : param <i>sort_by</i> invalid !</p>"; }
	  ...
	  // Push max value on sort table, only if respects days limit
	...
	// Afficher le formulaire
	  ...
	  // On parcourt toutes les identités
	  ...
	  for (var i=0;i<idtysListOrdered.length;i++)
	  {
		...
		+"<br>->"+idtysListOrdered[i]['nbValidCert'];
  ...
  ***************************************/
  ...
  app.get('/members', (req, res) => co(function *() {
	...
	for (let m=0;m<membersList.length;m++)
	{
	  ...
	  // Calculer le nombre de certifications reçues par le membre courant
	...
	{	  
	  ...
	  contenu += '>Inclure les certifications en piscine.</form><br>';
		  ...
		  // On parcourt tout les membres pour afficher ceux dont la date d'expiration est dans moins de 'd' jours
		  var nbPrintMembers = 0;
		  ...
		  for (var i=0;i<membersListOrdered.length;i++)
		  {
			...
			// calculer la couleur à attribuer à cette certification (dégradé du vert au rouge)
			let colorSig="";
  ...
  *****************************************/
  ...
  app.get('/monetaryMass', (req, res) => co(function *() {
    try {
      // get GET parameters
      var begin = 2;// Default Value
      if( typeof(req.query.begin) != 'undefined' && begin >= 2) { begin = req.query.begin; }
      var end = -1;// Default Value is current timestamp
  ...
  *****************************************/
  ...
  app.get('/membersCount', (req, res) => co(function *() {
    try {
      // get GET parameters
      var begin = 0;// Default Value
      ...
      var end = -1;// Default Value is current timestamp
      if( typeof(req.query.end) != 'undefined' ) { end = req.query.end; }
      var step = 288;// Default Value is current timestamp
      if( typeof(req.query.step) != 'undefined' && req.query.step > 0) { step = req.query.step; }
