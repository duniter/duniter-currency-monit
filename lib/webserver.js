  ...
  app.get('/willMembers', (req, res) => co(function *() {
	...
	hideIdtyWithZeroCert = req.query.hideIdtyWithZeroCert;
	  ...
	  for (var j=0;j<nbPendingCertifs;j++)
	  {
		...
		if (tmpQueryPendingCertifsList[j].expires_on > currentTimestamp)
		{
	  ...
	  // Stocker les informations de l'identité, sauf si elle doit être masquée (option masquage identités avec zéro certifs)
	  if (hideIdtyWithZeroCert != "yes" || nbValidPendingCertifs > 0)
	  {
	...
	{ 
      ...
      {
	  var maxTime = -1;
	  if (order == 'asc') { maxTime = currentTimestamp + (idtyWindow*2); }
	  var idMaxTime =0;
	...
	// Afficher le formulaire
	  ...
	  contenu += '<option name="sort_by" value ="creationIdty">date de création de l\'identité';
	  contenu += '<option name="sort_by" value ="expireIdty"';
	  if (sort_by == 'expireIdty' ) { contenu += ' selected'; }
	  contenu += '>date d\'expiration de l\'identité';
	  contenu += '<option name="sort_by" value ="sigCount"';
	  if (sort_by == 'sigCount' ) { contenu += ' selected'; }
	  contenu +='>nombre de certifications reçus';
	  ...
	  contenu += '</form><br>';//contenu += '>Masquer les identités n\'ayant reçus aucunes certifications.</form><br>';
	  
	  // Afficher le currentTimestamp
	  contenu += '<i>Nous sommes le '+timestampToDatetime(currentTimestamp)+'.</i><br><br>';
	  
	  // On parcour toutes les identités
	  var nbPrintMembers = 0;
	  contenu += '<b>Identités qui vont expirées dans moins de '+days+' jours :</b><table border=1>';
	  for (var i=0;i<idtysListOrdered.length;i++)
	  {
	    if (idtysListOrdered[i]['expires_on'] < limitTimestamp && idtysListOrdered[i]['expires_on'] > currentTimestamp)
	    {
		...
		contenu += "</tr>";
	      ...
	      let idtyProportion = ((idtysListOrdered[i]['expires_on']-currentTimestamp)*200)/idtyWindow;
		...
		+parseInt(idtyProportion).toString(16)+parseInt(idtyProportion).toString(16);
	      ...
	      // Printer la ligne
	      contenu += "<tr><td align='center' style=\"background:"+colorPendingIdty+"\">"+idtysListOrdered[i]['uid'].substring(0, 20)
		+"<br>"+idtysListOrdered[i]['pubkey'].substring(0, 8);
		...
		for (let j=0;j<idtysListOrdered[i]['pendingCertifications'].length;j++)
		{
		  // Calculer la proportion de temps restant avant expiration de la certification(en pour 255ème)
			  let pendingSigProportion = ((idtysListOrdered[i]['pendingCertifications'][j]['timestampExpire']-currentTimestamp)*200)/sigWindow;
		  ...
		  // Printer la certification
			  ...
			  +"<br>[inscriptible "+timestampToDatetime(idtysListOrdered[i]['pendingCertifications'][j]['timestampWritable'])+"]"
			  +"</td>";
		}
  ...
  app.get('/members', (req, res) => co(function *() {
	...
	for (let m=0;m<membersList.length;m++)
	{
	  ...
	  for (var i=0;i<nbWrittenCertifs;i++)
	  {
	    ...
	    if (tmpQueryCertifsList[i].expires_on > currentTimestamp)
	    {
	      if (i == 0)
	      {
	  ...
	  if (pendingSigs == "yes")
	  {
		...
		// Stoker la liste des certifications en piscine qui n'ont pas encore expirées
		if (tmpQueryPendingCertifsList[i].expires_on > currentTimestamp)
		{
	...
	else
	{
	  ...
	  contenu += '>Inclure les certifications en piscine.</form><br>';
		  ...
		  for (var i=0;i<membersListOrdered.length;i++)
		  {
			...
			{
			  // Calculer la proportion de temps restant avant expiration de la certification(en pour 255ème)
			  let pendingSigProportion = ((membersListOrdered[i]['pendingCertifications'][j]['timestampExpire']-currentTimestamp)*200)/sigWindow;
			  if (pendingSigProportion < 0 ) { pendingSigProportion = 0; }
			  else if (pendingSigProportion > 200 ) { pendingSigProportion = 200; }
