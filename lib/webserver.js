  ...
  ***************************************/
  ...
  app.get('/willMembers', (req, res) => co(function *() {
	...
	hideIdtyWithZeroCert = req.query.hideIdtyWithZeroCert;
	  ...
	  for (var j=0;j<nbPendingCertifs;j++)
	  {
	    ...
	    if ( tmpQueryGetUidIssuerPendingCert.length > 0 )
	    {
	      ...
	      let tmpQueryLastIssuerCert = yield duniterServer.dal.peerDAL.query('SELECT `chainable_on` FROM c_index WHERE `issuer`=\''+tmpQueryPendingCertifsList[j].from+'\' ORDER BY `expires_on` DESC LIMIT 1');
	...
	{ 
	  ...
	  { tabSort.push(idty.BlockNumber); }
	}
	else if (sort_by == "expireIdty")
	{
	  for (const idty of identitiesList)
	...
	{ 
      ...
      // Trier les identités par ordre croissant/decroissant du critère sort_by
      for (var i=0;i<identitiesList.length;i++)
      {
	  var maxTime = -1;
	  if (order == 'asc') { maxTime = currentBlockchainTimestamp + (idtyWindow*2); }
	  var idMaxTime =0;
	  ...
	  if (limitTimestamp > identitiesList[idMaxTime].expires_on)
	  {
	    // Vérifier que cette identité n'a pas déjà été prise en compte (empecher les doublons)
	    let doubloon = false;
	    for (const idty of idtysListOrdered)
	    {
	      if (identitiesList[idMaxTime].uid == idty.uid && identitiesList[idMaxTime].BlockNumber == idty.BlockNumber)
	      { doubloon = true; }
	    }
	    ...
	    if (!doubloon)
	    {
		  ...
		  uid: identitiesList[idMaxTime].uid,
		  creationTimestamp: identitiesList[idMaxTime].creationTimestamp,
		  pubkey: identitiesList[idMaxTime].pubkey,
		  BlockNumber: identitiesList[idMaxTime].BlockNumber,
		  expires_on: identitiesList[idMaxTime].expires_on,
		  pendingCertifications: idtysPendingCertifsList[idMaxTime]
	      });
	    }
	  }
	...
	// Afficher le formulaire
	  ...
	  contenu += '<option name="sort_by" value ="creationIdty">date d\'expiration de l\'identité';
	  ...
	  contenu += ' <input type="submit" value="envoyer"><br>';
	  
	  contenu += '<input type="checkbox" name="hideIdtyWithZeroCert" value="yes"';
	  if (hideIdtyWithZeroCert == 'yes' ) { contenu += ' checked'; }
	  contenu += '>Masquer les identités n\'ayant reçu aucune certification.</form><br>';
	  
	  // Afficher le currentBlockchainTimestamp et la légende
	  contenu += '<i><font color=\'DarkRed\'>darkred : [date à partir de laquelle cette certification pourra être écrite compte tenu de sigPeriod].</font><br>';
	  contenu += '<font color=\'green\'>green : [disponible] : signifie que cette certification pourrait être écrite dès le prochain block.</font></i><br><br>';
	  contenu += '<b>À quel moment une identité devient membre ?</b><br>';
	  contenu += 'Une identité devient membre dès qu\'elle bénéficie d\'au moins <b>5</b> certifications à l\'état <font color=\'green\'>[disponible]</font> <b>au même moment <u>et</u></b> qu\'elle respecte la règle de distance.<br>';
	  contenu += 'Vous pouvez cliquer directement sur une identité pour arriver sur la page wotex correspondante afin de vérifier si elle respecte la règle de distance.<br>';
	  contenu += '<br>';
	  contenu += '<b>C\'est quoi la règle de distance ?</b><br>';
	  contenu += 'Il faut que pour plus de 80% des membres référents, il existe au moins un chemin de 5 pas ou moins entre eux et l\'identité considérée.<br>';
	  contenu += 'Tout membre devient automatiquement référent dés lors qu\'il à émis <b><u>et</u></b> reçu Y[n] certifications. (Y[n] varie en fonction du nombre total de membres, la formule exacte est affichée sur cesium).<br>';
	  contenu += '<br>';
	  contenu += '<b>Temps Blockchain actuel : '+timestampToDatetime(currentBlockchainTimestamp)+'.</b><br>';
	  
	  // On parcour toutes les identités
	  var nbPrintMembers = 0;
	  contenu += 'Identités qui vont expirer dans moins de '+days+' jours :<table border=1>';
	  for (var i=0;i<idtysListOrdered.length;i++)
	  {
	      ...
	      && (hideIdtyWithZeroCert != "yes" || idtysListOrdered[i]['pendingCertifications'].length > 0) )
	    {
	      ...
	      if (nbPrintMembers == 0) {
		contenu += "<tr><td align='center'>uid</td><td align='center'>création indentité</td>";
		contenu += "<td align='center'>expiration identité</td><td style=\"background:#000000\">-</td>";
		...
		for (let j=0;j<idtysListOrdered[i]['pendingCertifications'].length;j++)
		{
		  ...
		  // Printer la certification
			  contenu += "<td align='center' style=\"background:"+colorPendingSig+"\">"+idtysListOrdered[i]['pendingCertifications'][j]['from']
			  +"<br>"+timestampToDatetime(idtysListOrdered[i]['pendingCertifications'][j]['timestampExpire'])
			  +"<br>émise #"+idtysListOrdered[i]['pendingCertifications'][j]['blockNumber'];
			  ...
			  else { contenu += "<br><font color='green'>[disponible]</font>"; }
			  contenu += "</td>";
		}
	  ...
	  contenu += "<tr><td colspan="+(4+nbMaxCertifs)+" align='center'> total : <b>"+nbPrintMembers+"</b> identités.</td><t/tr></table><br><hr>";
  ...
  ***************************************/
  ...
  app.get('/members', (req, res) => co(function *() {
	...
	else
	{
	  ...
	  yield htmlRendering(res, sigValidity, msValidity, currentBlockchainTimestamp, days, sort_by, order,
