...
module.exports = (req, res, HTML_HEADERS, htmlMenu, tabCurrency, begin, end) => {
  if (typeof(req.query.format) != 'undefined' && req.query.format == 'JSON')
  {
    ...
    if (typeof(req.query.unit) != 'undefined' && req.query.unit == 'relativ') { unit = 'relativ'; }
	...
	else if (unit == 'relativ')
	{
	    tabLabels.push(tabCurrency[i].dateTime);
	    if (massByMembers == "no")
	    {
	      tabMonetaryMass.push(tabCurrency[i].relativMonetaryMass);
	    }
	    else
	    {
	      tabMonetaryMass.push(tabCurrency[i].relativMonetaryMassPerMembers);
	    }
	}
    ...
    contenu += '<option name="unit" value ="relativ"';
    if (unit == 'relativ' ) { contenu += ' selected'; }
    contenu +='>relativ';
