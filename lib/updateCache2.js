...
const constants2_1 = require("./constants2");
const co = require('co');
/**
     * updateCache
     *
...
module.exports = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        upgradeCache = (Math.floor(Date.now() / 1000) > (cache.lastUptime + constants2_1.MonitConstants.MIN_CACHE_UPDATE_FREQ));
        ...
        if (cache.endBlock != null) {
            let checkBlock = [yield dataFinder.getBlock(cache.blockchain[cache.blockchain.length - 1].number)];
        ...
        // define step
        cache.step = 1;
        if (typeof (req.query.step) != 'undefined' && parseInt(req.query.step) > 0) {
            cache.step = req.query.step;
        }
        ...
        if (typeof (req.query.stepUnit) != 'undefined') {
            switch (req.query.stepUnit) {
                case "blocks":
                    unitTime = 300;
                    cache.onlyDate = false;
                    cache.stepUnit = "blocks";
                    break;
                case "hours":
                    unitTime = 3600;
                    cache.onlyDate = false;
                    cache.stepUnit = "hours";
                    break;
                case "days":
                    unitTime = 86400;
                    cache.onlyDate = true;
                ...
                case "weeks":
                    unitTime = 604800;
                    cache.onlyDate = true;
                    cache.stepUnit = "weeks";
                    break;
                case "months":
                    unitTime = 18144000;
                    cache.onlyDate = true;
                    cache.stepUnit = "months";
                    break;
                case "years":
                    unitTime = 31557600;
                    cache.onlyDate = true;
                    cache.stepUnit = "years";
                    break;
            }
        }
        // Default values
        ...
        if (typeof (req.query.end) == 'undefined' || req.query.end <= 0) {
            cache.endBlock = [yield dataFinder.getCurrentBlockOrNull()];
        }
        else {
            cache.endBlock = [yield dataFinder.getBlock(req.query.end)];
        ...
        if (typeof (req.query.begin) == 'undefined' || req.query.begin < 0) {
            cache.beginBlock = [yield dataFinder.getBlock(0)];
        }
        else if (req.query.begin > cache.endBlock[0].number) {
            let beginTime = cache.endBlock[0].medianTime - (parseInt(cache.step) * unitTime * constants2_1.MonitConstants.STEP_COUNT_MIN);
            cache.beginBlock = yield dataFinder.getBlockWhereMedianTimeGte(beginTime);
        }
        else {
            cache.beginBlock = [yield dataFinder.getBlock(req.query.begin)];
        }
        ...
        if (typeof (req.query.nbMaxPoints) != 'undefined' && req.query.nbMaxPoints > 0) {
            ...
            cache.nbMaxPoints = constants2_1.MonitConstants.STEP_COUNT_MAX;
        }
        ...
        if (cache.adaptMaxPoints == "begin") {
            if (Math.ceil((cache.endBlock[0].medianTime - cache.beginBlock[0].medianTime) / (cache.step * unitTime)) > cache.nbMaxPoints) {
                ...
                cache.beginBlock = yield dataFinder.getBlockWhereMedianTimeGte(newBeginTime);
            }
        }
        else if (cache.adaptMaxPoints == "step") {
            cache.step = Math.ceil((cache.endBlock[0].medianTime - cache.beginBlock[0].medianTime) / (constants2_1.MonitConstants.STEP_COUNT_MAX * unitTime));
        }
        else {
            let newEndTime = cache.beginBlock[0].medianTime + cache.step * cache.nbMaxPoints * unitTime;
            cache.endBlock = yield dataFinder.getBlockWhereMedianTimeLte(newEndTime);
        }
        // Calculate stepTime
        cache.stepTime = parseInt(cache.step) * unitTime;
        // if new blocks and MIN_CACHE_UPDATE_FREQ pass, update cache
        if (parseInt(cache.endBlock[0].number) >= cache.currentBlockNumber && Math.floor(Date.now() / 1000) > (cache.lastUptime + constants2_1.MonitConstants.MIN_CACHE_UPDATE_FREQ)) {
            // let previousCacheTime = (cache.blockchain.length > 0) ? cache.blockchain[cache.blockchain.length-1].medianTime:0;
            var newBlocks = yield dataFinder.getBlockWhereMedianTimeLteAndGtNoLimit(cache.currentBlockTime, cache.endBlock[0].medianTime);
            ...
            for (let b = 0; b < newBlocks.length; b++) {
                ...
                if (newYn > cache.Yn && cache.Yn > 0) {
                    for (let m = 0; m < cache.pubkeys.length; m++) {
                        if (cache.pubkeys[m].writtenCerts.length >= cache.Yn && cache.pubkeys[m].receivedCerts.length >= minReceivedCerts && (cache.pubkeys[m].writtenCerts.length < newYn || cache.pubkeys[m].receivedCerts.length < newYn)) {
                            newSentries -= 1;
                        }
                    }
...
//# sourceMappingURL=updateCache2.js.map